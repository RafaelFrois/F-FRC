...
function recalculateUserWithScoreMap(user, scoreMap) {
  ...
  for (const regional of regionals) {
    ...
    for (const member of regional.alliance) {
      ...
      regionalTotal += applyCaptainMultiplier(points, Boolean(member?.isCaptain));
    }
  ...
  const currentWeekPoints = regionals.reduce((sum, regional) => {
    return sum + Number(regional?.totalRegionalPoints || 0);
  }, 0);

  const carryover = Number(user?.totalPointsCarryover || 0);
  const totalPointsSeason = carryover + currentWeekPoints;
...
export async function refreshSingleUserScores(user) {
  ...
  if (eventKeys.length === 0) {
    const carryover = Number(user?.totalPointsCarryover || 0);
    if (Number(user?.totalPointsSeason || 0) !== carryover) {
      user.totalPointsSeason = carryover;
      return true;
    }
    return false;
  }
